...
function Cards({ data }) {
  ...
  const renderProduct = (product, id) => {
   ...
   return isInCart ? (
      ...
      <div
        className="absolute top-0 right-0 bg-black text-white text-lg px-2 py-1 flex justify-center items-center w-8 h-6 rounded-full m-2"
        onClick={(e) => {
  ...
  return (
    <div className=" cursor-pointer w-full h-full  rounded-lg flex justify-center items-center  flex-wrap gap-28 mt-10">
      {/* eslint-disable-next-line react/prop-types */}
      {data.map((product) => (
        <div
          key={product.id}
          className="bg-transparent cursor-pointer w-66 h-80  rounded-lg"
        ...
        >
          <figure className="relative mb-2 w-full h-4/5">
            ...
            <img
              className="bg-transparent w-56 h-full object-cover rounded-lg"
              src={product.image}
              alt={product.title}
            />
